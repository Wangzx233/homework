...
//留言板插入数据
func M_insert(content string,from_uid string,to_id string) {
	initdb()
	defer db.Close()
	sqlStr:=`insert into message (content,from_uid,to_id) values(?,?,?);`
	_,err:=db.Exec(sqlStr,content,from_uid,to_id)
	if err!=nil{
		log.Println(err)
		return
	}
