...
func Register(uid string,name string,password string) bool {
	...
	R_update(uid,name,password)
	return true
}
//新建用户
func R_update(uid,name,password string)  {

	_,err:=DB.Exec("insert user (uid,name,password) values (?,?,?)",uid,name,password)
	if err!=nil{
